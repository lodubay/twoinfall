...
As stellar age estimation techniques have improved over recent years, large catalogs have become available with measurements for stars across a wide swath of the Galaxy. In a challenge to the traditional view of GCE, which expects the ISM metallicity to continually increase over time, 
...
\begin{table}
    ...
    \input{output/uncertainties.tex}
\end{table}
...
\begin{deluxetable*}{Ccccl}
    ...
    \startdata
        ...
        % \Delta R_{\rm gal}  & ---       & ---               & \ref{sec:migration}   & Change in orbital radius due to stellar migration \\
        ...
        % p(z|\tau,R_{\rm final}) & Equation \ref{eq:sech-pdf}    & ---   & \ref{sec:migration} & Probability density function of Galactic midplane distance\\
...
\begin{deluxetable*}{lCCcCCC}
    ...
    \tablehead{
        \colhead{Name} & \colhead{$y/Z_\odot$} & \colhead{$\eta_\odot$} & \colhead{SN Ia DTD} & \colhead{$\sigma_{\rm RM8}$} & \colhead{$f_\Sigma(R_\odot)$} & \colhead{${\rm [X/H]}_{\rm CGM}$}
    }
    \startdata
...
% We intend this model to illustrate another path to reproducing the $\alpha$-bimodality. Most of the high-$\alpha$ stars present in the Solar annulus have likely migrated from the inner Galaxy, where perhaps this SFE-driven hiatus was concentrated.
...
The figures, tables, and models in this paper are reproducible. The git repository associated to this study is publicly available at \url{\GitHubURL}, and the release \todo{v1.X.X} allows anyone to re-build the entire manuscript. The multi-zone model outputs are stored at \todo{Zenodo URL}, and the source code for this manuscript is stored at \todo{Zenodo URL}.
