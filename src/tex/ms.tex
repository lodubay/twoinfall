...
\begin{deluxetable*}{Cccl}
    ...
    \startdata
        ...
        % Outflows
        R_\eta          & 5.4 kpc   & \ref{sec:outflows}    & Outflow scale radius \\
        % Yields
        ...
        t_{\rm on}      & 4.2       & \ref{sec:sfh}     & Onset time of the second infall epoch \\
        f_{\Sigma,0}    & 0.27      & \ref{sec:sfh}     & Central thick/thin disk surface density ratio \\
        {\rm [X/H]}_{\rm CGM}   & Pristine  &           & Pre-enrichment of infalling gas
    \enddata
\end{deluxetable*}
\vspace{-24pt}
...
\begin{equation}
    \eta_\odot = \frac{y_{\rm O}^{\rm CC}}{Z_{\rm O,\odot}} - 1 + r
\end{equation}
...
We run {\tt VICE} in ``infall mode,'' where we specify the gas infall density $\dot\Sigma_{\rm in}$ and the star formation effieciency (SFE) timescale $\tau_\star\equiv \Sigma_g / \dot\Sigma_\star$. The gas surface density $\Sigma_g$ and star formation rate $\dot\Sigma_\star$ are calculated from the two specified quantities according to our star formation law, which is described in Section \ref{sec:sf-law}, and assuming zero initial gas mass in all zones.
...
\end{equation}
where $\tau_1$ and $\tau_2$ are the first and second infall timescales, respectively, $t_{\rm on}$ is the onset time of the second infall epoch, and $f_{2/1}$ is the ratio of the second infall amplitude to the first. We calculate $f_{2/1}$ for each zone such that the resulting stellar density profile follows a two-component disk, with the surface density ratio of the thick and thin disks given by
\begin{equation}
...
The parameters of Equation \ref{eq:infall-rate} have an important effect on the model output. Previous studies of the two-infall model have adopted values for the infall parameters of $\tau_1\sim0.1-1$ Gyr, $\tau_2\sim4-10$ Gyr, and $t_{\rm on}\sim4$ Gyr. Figure \ref{fig:twoinfall-parameters} illustrates the effect of varying each of these parameters on gas abundance tracks and stellar abundance distributions in a one-zone model. The first infall timescale $\tau_1$ primarily affects the stellar distribution along the high-$\alpha$ sequence, with negligible effect on the low-$\alpha$ sequence (a). On the other hand, the second infall timescale $\tau_2$ controls the width of the MDF and the [O/Fe] distribution of the low-$\alpha$ sequence, with a shorter $\tau_2$ producing a distribution which is skewed to higher [O/Fe] (b). The onset time $t_{\rm on}$ strongly affects the overall stellar abundance distribution for values $t_{\rm on}\leq2$ Gyr, but in this case the gas tracks do not produce the characteristic loop characteristic of the two-infall model. For $t_{\rm on}>2$ Gyr, varying $t_{\rm on}$ results in a minor shift to the mean of the MDF and little change to the [O/Fe] distributions, even though the abundance tracks in [O/Fe]--[Fe/H] space appear very different. We find that $\tau_2$ is the most important parameter to tune in order to reproduce the observed stellar abundance distributions.
...
\begin{equation}
    p(z|\tau,R_{\rm final}) = \frac{1}{4 h_z} {\rm sech}^2\Big(\frac{z_{\rm final}}{2 h_z(\tau,R_{\rm final})}\Big).
    \label{eq:sech-pdf}
\end{equation}

\begin{equation}
    h_z(\tau,R_{\rm final}) = (h_{z,s}/e^2) \exp(\tau/\tau_s + R_{\rm final}/R_s)
...
\begin{figure*}
    \centering
    \includegraphics{src/tex/figures/ofe_df_yields.pdf}
