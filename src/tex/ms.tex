...
\todo{Integrate into previous para:} \citet{nissen_high-precision_2020} and \citet{nataf_accurate_2024} observed multiple sequences in the local age--metallicity relation (AMR), which could be interpreted as evidence supporting the two-infall scenario, but \citet{plotnikova_chemical_2024} argued that such patterns can arise due to selection effects.
\todo{Also: multiple sequences in Anders et al. (2023), Xiang \& Rix (2022), Sahlholdt et al. (2022).}
...
\begin{figure}
    ...
    The left-hand marginal panels show the predicted (blue, pink, and green) and observed (gray) stellar abundance distributions \added{in the same region}, which are boxcar-smoothed with a width of 0.05 dex for visual clarity.}
    \label{fig:yield-outflow}
\end{figure}

Figure \ref{fig:yield-outflow} shows the \added{APOGEE age--abundance distributions at the Solar annulus, as well as the binned mode abundance and rolling median as a function of age}, using the NN ages from \citet{leung_variational_2023}. As shown by \citet{johnson_milky_2025}, the peak of the MDF is less susceptible to modification by radial migration, making it a more reliable proxy for ISM chemistry at a given lookback time than the mean or median. \added{While the age--metallicity distribution is clumpy in places (see Section \ref{sec:amr}), we observe that ${\rm mode}(\mathOH)\gtrsim0$ for the past 10 Gyr.}
...
\begin{figure*}
    ...
    \caption{Comparison between NN- and [C/N]-derived ages for our sample.}
