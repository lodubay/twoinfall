...
\added{A few recent studies have reported the discovery of multiple disconnected sequences in the local age--metallicity relation \citep[e.g.,][]{sahlholdt_characterizing_2022,xiang_time-resolved_2022,anders_spectroscopic_2023}. \citet{nissen_high-precision_2020} argued that these sequences provide evidence for the two-infall scenario, although \citet{plotnikova_chemical_2024} showed that multiple sequences can arise due to selection effects, and \citet{chen_recent_2025} showed that radial migration can also lead to such patterns. There are hints of multiple sequences in Figure \ref{fig:yield-outflow}, where a break in the distribution at ages of $\sim5\Gyr$ coincides with a $\sim0.2\dex$ drop in [O/H] and [Fe/H]. The sequences are less pronounced in the \citet{leung_variational_2023} catalog than reported by \citet{anders_spectroscopic_2023}, likely related to a systematic offset between the two catalogs around the same age \citep[see Figure 6 of][]{anders_spectroscopic_2023}. On the other hand, the [C/N] age--metallicity relation of \citet{roberts_cn_2025} is even flatter and does not appear to have multiple sequences (Figure \ref{fig:compare-age-catalogs}). Even if these sequences are real, we find it difficult to explain them using the two-infall scenario\textemdash the observed dilution is much smaller than predicted by any of our models, and the timing of the split between the sequences is too late for the birth of the thin disk, although it is interestingly close to the first pericentric passage of the Sagittarius dwarf galaxy \citep{ruiz-lara_recurrent_2020}.}
\todo{Mention APOGEE selection function?}
...
\todo{Even if the Anders et al. (2023) sequences are right, it still doesn't look like two-infall. The Anders AMR reaches Solar-ish metallicity 8 Gyr ago, then drops to $\mathFeH\approx-0.2$ by 4 Gyr ago. A 0.2 dex dilution over 4 Gyr is {\it far} weaker and more gradual than predicted by any of our two-infall models.}
