...
% Recently, \citet{weinberg_scale_2024} used a measurement of the mean Fe yield of core-collapse supernovae (CCSNe) by \citet{rodriguez_iron_2023} to infer a supernova yield scale that is a factor of $\sim2$ lower than assumed by some previous GCE studies.
...
% However, the high-yield models experience a decline in [O/Fe] of $\sim0.2$ dex between the second infall and the present day, contrasted with the smaller decline of $\sim0.1$ dex in the low-yield model.
...
% We adopt pre-enriched infall ($\mathFeH_{\rm CGM}=-0.7$), stronger radial migration ($\sigma_{\rm RM8}=3.6$ kpc), and a higher disk mass ratio ($f_\Sigma(R_\odot)=0.25$) because these parameters were able to improve agreement with observations in previous sections. 
...
% This over-density occurs because the overall rate of chemical evolution slows down $\sim2\,{\rm Gyr}$ after the second infall, and at the same time the delayed enrichment from SNe Ia reverses the evolution of [O/Fe]. 
...
% The precise location of the cut matters little, as we observe the same behavior for cuts ranging from $+0.05$ to $+0.2$ dex. 
...
% The \yZ{1} empirical yield scale already has difficulties matching the local age--metallicity relation \citep[see also][]{johnson_milky_2024}, but the problem is exacerbated in the two-infall case because of the delayed dilution event --- in effect, approach to equilibrium is ``reset'' by the second infall.
...
% Radial gas flows allow GCE models to produce a radial metallicity gradient in the absence of mass-loaded outflows.
